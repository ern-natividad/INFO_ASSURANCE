...
function getPasswordStrength(password) {
  ...
  let level, color;
  if (score <= 2) { level = "Weak"; color = "#dc2626"; }
  else if (score <= 4) { level = "Fair"; color = "#ea580c"; }
  else if (score <= 6) { level = "Good"; color = "#ca8a04"; }
...
export default function SignUp({ }) {
  ...
  function validate() {
    ...
    if (!isAlphanumeric(password))
      return { ok: false, message: "Password must be alphanumeric" };
    ...
    if (containsSqlLike(password) || containsSqlLike(username) || containsXssLike(password) || containsXssLike(username))
      ...
      <form className="login-form" onSubmit={handleSubmit} noValidate>
        ...
        <label className="field">
          ...
          <div style={{ position: "relative", display: "flex", alignItems: "center" }}>
            ...
            />
            <button type="button" onClick={() => setShowPassword(!showPassword)} style={{ position: "absolute", right: "12px", background: "none", border: "none", cursor: "pointer", fontSize: "18px", color: "#94a3b8" }}>
              {showPassword ? "👁️" : "👁️‍🗨️"}
            </button>
          </div>
          {password && (
            <div style={{ marginTop: "8px" }}>
              <div style={{ height: "6px", backgroundColor: "#e5e7eb", borderRadius: "3px", overflow: "hidden" }}>
                <div style={{ height: "100%", width: `${(passwordStrength.score / passwordStrength.maxScore) * 100}%`, backgroundColor: passwordStrength.color, transition: "width 0.3s" }} />
              </div>
              <span style={{ fontSize: "12px", fontWeight: "700", color: passwordStrength.color }}>{passwordStrength.level}</span>
            </div>
          )}
        </label>
        ...
        <label className="field">
          ...
          <div style={{ position: "relative", display: "flex", alignItems: "center" }}>
            <input
              ...
              style={{ paddingRight: "45px" }}
            />
            <button type="button" onClick={() => setShowConfirmPassword(!showConfirmPassword)} style={{ position: "absolute", right: "12px", background: "none", border: "none", cursor: "pointer", fontSize: "18px", color: "#94a3b8" }}>
        ...
        <div style={{ textAlign: "center", marginTop: "1.5rem", fontSize: "0.9rem", color: "#64748b" }}>
          ...
          <button type="button" onClick={() => navigate('/login')} style={{ background: "none", border: "none", color: "#2563eb", cursor: "pointer", fontWeight: "600", marginLeft: "5px" }}>
            Sign in
          </button>
        </div>
      </form>
