...
export default function SQLInjection() {
  ...
  const mockDatabase = [
    { id: 101, name: 'Villanueva', position: 'Manager', salary: '$85,000', department: 'Sales' },
    { id: 102, name: 'Ignacio', position: 'Developer', salary: '$70,000', department: 'IT' },
    { id: 103, name: 'Natividad', position: 'CEO', salary: '$250,000', department: 'Executive' },
  ];
  ...
  const lessons = {
    1: {
      ...
      query: (val) => `SELECT * FROM employees WHERE last_name = '${val || '____'}'`,
      check: (val) => val.includes("' OR '1'='1") || val.includes("' or '1'='1")
    },
    2: {
      title: "Lesson 2: Numeric Injection",
      goal: "Numeric fields don't use quotes. Bypass the ID check.",
      query: (val) => `SELECT * FROM employees WHERE user_id = ${val || '____'}`,
    ...
    3: {
      ...
      query: (val) => `SELECT * FROM employees WHERE last_name = '${val || '____'}' AND status = 'ACTIVE'`,
  ...
  const handleFetchData = () => {
    ...
    if (isInjected) {
      ...
      const found = mockDatabase.filter(emp => 
        activeLesson === 2 ? emp.id.toString() === userInput : emp.name.toLowerCase() === userInput.toLowerCase()
      );
      setInjectionResult({
      ...
      <div className="lesson-content fade-in-content">
        <div className="sql-header">
          ...
          <div className="lesson-tabs">
            <button className={activeLesson === 1 ? 'tab-btn active' : 'tab-btn'} onClick={() => switchLesson(1)}>1. String SQLi</button>
            <button className={activeLesson === 2 ? 'tab-btn active' : 'tab-btn'} onClick={() => switchLesson(2)}>2. Numeric SQLi</button>
            <button className={activeLesson === 3 ? 'tab-btn active' : 'tab-btn'} onClick={() => switchLesson(3)}>3. Commenting</button>
          </div>
        </div>
        ...
        <div className="sql-info-grid">
          ...
          <div className="sql-card impact">
            <h4>How to Solve</h4>
            <p>
              {activeLesson === 1 && <span>Try breaking the string: <code>' OR '1'='1</code></span>}
              {activeLesson === 2 && <span>No quotes needed here! Try: <code>1 OR 1=1</code></span>}
              {activeLesson === 3 && <span>Break the string and comment out the rest: <code>' OR 1=1 --</code></span>}
            </p>
          </div>
        </div>
        ...
        <div className="sql-lab-box">
          <div className="lab-header">
            <h4>{activeLesson === 2 ? "Search by Employee ID" : "Search by Last Name"}</h4>
          </div>
          
          <div className="lab-layout">
            <div className="lab-form">
              <div className="sql-input-group">
                <label>{activeLesson === 2 ? "Enter ID:" : "Enter Last Name:"}</label>
                <input 
                  type="text" 
                  value={userInput}
              ...
              <button className="sql-btn-test" onClick={handleFetchData}>Execute Query</button>
            </div>
          ...
          {injectionResult && (
            <div className={`sql-result-area ${injectionResult.status.includes('ℹ️') ? 'danger' : 'success'}`}>
              <h5>{injectionResult.status}</h5>
              <code className="query-log">{injectionResult.message}</code>
              {injectionResult.data.length > 0 && (
                <table className="results-table">
                  <thead><tr><th>ID</th><th>Name</th><th>Position</th><th>Salary</th></tr></thead>
                  <tbody>
                    {injectionResult.data.map((emp, i) => (
                      <tr key={i}>
                        ...
                        <td>{emp.name}</td>
                        <td>{emp.position}</td>
                        <td className={injectionResult.status.includes('🚨') ? 'red-text' : ''}>{emp.salary}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
